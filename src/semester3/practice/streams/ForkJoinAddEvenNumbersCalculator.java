...
public class ForkJoinAddEvenNumbersCalculator extends RecursiveTask<Long> {
    ...
    protected Long compute() {
        ...
        /*
         * Hier wird das Wissen ausgenutzt, dass jede zweite Zahl gerade ist Die letzte
         * gerade Zahl von leftTask hängt von start ab: start gerade => start +
         * 2*[(anzahl/2)-1] = start + anzahl - 2 start ungerade => (start + 1) +
         * 2*[(anzahl/2)-1] = start + anzahl - 1 allgemein: start + anzahl - 2 + start%2
         * anzahl = anzahl/2 + [anzahl/2 + anzahl%2] => rightTask muss [anzahl/2 +
         * anzahl%2] gerade Zahlen addieren
         */

        ForkJoinAddEvenNumbersCalculator rightTask = new ForkJoinAddEvenNumbersCalculator(
                start + anzahl - 2 + start % 2 + 1, anzahl / 2 + anzahl % 2);
